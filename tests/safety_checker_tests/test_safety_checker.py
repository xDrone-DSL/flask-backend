...
class SafetyCheckerTest(unittest.TestCase):
    ...
    def test_check_takeoff_should_update_state(self):
        state = State.init_state()
        SafetyChecker._check_takeoff(Command.takeoff(), state, self.drone_config, self.safety_config)
    ...
    def test_check_takeoff_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=0, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_takeoff(Command.takeoff(), state, self.drone_config, safety_config)
    ...
    def test_check_land_should_update_state(self):
        state = State(has_taken_off=True, z_meters=3)
        SafetyChecker()._check_land(Command.land(), state, self.drone_config, self.safety_config)
        expected = State(has_taken_off=False, time_used_seconds=3 / 0.5, z_meters=0)
        self.assertEqual(expected, state)

    def test_check_land_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            state = State(has_taken_off=True, time_used_seconds=2, z_meters=3)
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_land(Command.land(), state, self.drone_config, safety_config)
    ...
    def test_check_up_should_update_state(self):
        state = State(has_taken_off=True, z_meters=3)
        SafetyChecker()._check_up(Command.up(1), state, self.drone_config, self.safety_config)
    ...
    def test_check_up_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_up(Command.up(2), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 2 / 0.5)
                        in str(context.exception))
    ...
    def test_check_down_should_update_state(self):
        state = State(has_taken_off=True, z_meters=3)
        SafetyChecker()._check_down(Command.down(1), state, self.drone_config, self.safety_config)
        expected = State(has_taken_off=True, time_used_seconds=1 / 0.5, z_meters=3 - 1)
        self.assertEqual(expected, state)
    ...
    def test_check_down_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_down(Command.down(2), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 2 / 0.5)
                        in str(context.exception))
    ...
    def test_check_left_should_update_state(self):
        state = State(has_taken_off=True, x_meters=1, y_meters=2, z_meters=3, orientation_degrees=60)
        SafetyChecker()._check_left(Command.left(1), state, self.drone_config, self.safety_config)
    ...
    def test_check_left_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_left(Command.left(2), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 2 / 0.5)
                        in str(context.exception))
    ...
    def test_check_right_should_update_state(self):
        state = State(has_taken_off=True, x_meters=1, y_meters=2, z_meters=3, orientation_degrees=60)
        SafetyChecker()._check_right(Command.right(1), state, self.drone_config, self.safety_config)
        expected = State(has_taken_off=True, time_used_seconds=1 / 0.5, x_meters=1 + 0.5, y_meters=2 - sqrt(3) / 2,
                         z_meters=3, orientation_degrees=60)
        self.assertEqual(expected, state)
    ...
    def test_check_right_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_right(Command.right(2), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 2 / 0.5)
                        in str(context.exception))
    ...
    def test_check_forward_should_update_state(self):
        state = State(has_taken_off=True, x_meters=1, y_meters=2, z_meters=3, orientation_degrees=60)
        SafetyChecker()._check_forward(Command.forward(1), state, self.drone_config, self.safety_config)
    ...
    def test_check_forward_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_forward(Command.forward(2), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 2 / 0.5)
                        in str(context.exception))
    ...
    def test_check_backward_should_update_state(self):
        state = State(has_taken_off=True, x_meters=1, y_meters=2, z_meters=3, orientation_degrees=60)
        SafetyChecker()._check_backward(Command.backward(1), state, self.drone_config, self.safety_config)
        expected = State(has_taken_off=True, time_used_seconds=1 / 0.5, x_meters=1 - sqrt(3) / 2, y_meters=2 - 0.5,
                         z_meters=3, orientation_degrees=60)
        self.assertEqual(expected, state)
    ...
    def test_check_backward_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_backward(Command.backward(2), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 2 / 0.5)
                        in str(context.exception))
    ...
    def test_check_rotate_left_should_update_state(self):
        state = State(has_taken_off=True, x_meters=1, y_meters=2, z_meters=3, orientation_degrees=60)
        SafetyChecker()._check_rotate_left(Command.rotate_left(90), state, self.drone_config, self.safety_config)
    ...
    def test_check_rotate_left_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_rotate_left(Command.rotate_left(90), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 90 / 45)
                        in str(context.exception))
    ...
    def test_check_rotate_right_should_update_state(self):
        state = State(has_taken_off=True, x_meters=1, y_meters=2, z_meters=3, orientation_degrees=60)
        SafetyChecker()._check_rotate_right(Command.rotate_right(90), state, self.drone_config, self.safety_config)
        expected = State(has_taken_off=True, time_used_seconds=1 / 0.5, x_meters=1, y_meters=2,
                         z_meters=3, orientation_degrees=150)
        self.assertEqual(expected, state)
    ...
    def test_check_rotate_right_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_rotate_right(Command.rotate_right(90), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 90 / 45)
                        in str(context.exception))
    ...
    def test_check_wait_should_update_state(self):
        state = State(has_taken_off=True, x_meters=1, y_meters=2, z_meters=3, orientation_degrees=60)
        SafetyChecker()._check_wait(Command.wait(5), state, self.drone_config, self.safety_config)
    ...
    def test_check_wait_should_check_state(self):
        with self.assertRaises(SafetyCheckError) as context:
            ...
            safety_config = SafetyConfig(max_seconds=3, max_x_meters=5, max_y_meters=5, max_z_meters=5,
                                         min_x_meters=-5, min_y_meters=-5, min_z_meters=-5)
            SafetyChecker._check_wait(Command.wait(2), state, self.drone_config, safety_config)
        self.assertTrue("The time used {} seconds will go beyond the time limit 3 seconds"
                        .format(2 + 2)
                        in str(context.exception))
